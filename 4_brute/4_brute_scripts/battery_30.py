...
def simulate_battery_dispatch(
    ...
    roundtrip_eff: float = 0.9,      # Updated to 90%
    min_soc_pct: float = 0.05,       # Updated to 5% (depth of discharge 95%)
    annual_deg_rate: float = 0.01,   # Keep at 1% per year
    grid_emission_rate: float = 0.81 # Updated to 0.81 kg CO2e per kWh
) -> (pd.DataFrame, dict):
    """
    ...
    annual_deg_rate : float
        Fractional capacity fade per year.
    ...
    totals : dict
        Aggregated metrics:
          ...
          - grid_fraction           # total_import / total_demand
          - self_consumption_rate   # pv_used / (pv_used + pv_export)
    """
    # 1) Align series
    ...
    for i, (ts, pv_val, dem_val) in enumerate(zip(pv.index, pv, dem)):
        ...
        avail     = (soc - cur_capacity * min_soc_pct) * eff_dis
