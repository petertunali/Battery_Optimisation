...
def simulate_battery_dispatch(
    ...
    grid_emission_rate: float = 0.79  # 0.79 kg CO2e per kWh - DCCEW
) -> (pd.DataFrame, dict):
    """
    ...
    for i, (ts, pv_val, dem_val) in enumerate(zip(pv.index, pv, dem)):
        ...
        final_capacity = cur_capacity  # Keep track of final capacity
        ...
        soc += charge * eff_chg
        surplus -= charge
        total_charge += charge  # Track total charge for cycle calculation
    ...
    # 8) Calculate battery cycles
    battery_cycles = 0
    if battery_kwh > 0:
        battery_cycles = total_charge / battery_kwh
    
    # 9) Calculate final degradation percentage
