...
def compute_financials(
    ...
    """
    Compute CAPEX, OPEX, export revenue, import cost, IRR, and NPC.
    """
    # 1) CAPEX
    ...
    # 3) Import cost
    import_peak    = totals.get('total_grid_import_peak',   0.0) * import_price_peak
    import_offpeak = totals.get('total_grid_import_offpeak',0.0) * import_price_offpeak
    import_cost = import_peak + import_offpeak

    # 4) OPEX
    opex_total = opex_per_day * battery_kwh * 365 * years

    # 5) Net present cost (NPC)
    net_cost = capex_total + opex_total + import_cost - export_rev

    # 6) Annual cash flows for IRR
    ...
    # 8) NPV (if discount_rate/WACC provided)
    npv = None
